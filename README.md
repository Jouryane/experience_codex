...
**你的 Agent 不缺能力，缺的是可复现、可审计、可回滚,也就是被我们称为经验的东西**

Experience 是 Agent 与 AI应用/具身场景之间的**可验证执行层**（我们叫它第3.5层）：
已解决的问题由它在进程内直接完成并留下证据、部分固化为经验，而将未知的、杂糅的复杂任务交给 Agent。

30 秒看懂三件事：

1. **命中即先动手**——Experience不是给 Agent 提示词，而是执行预编译步骤（写入/追加/复制/
   移动/删除、受控命令），并自动备份；
...
Experience的初衷建立在，真实的神经系统不仅仅只有意识在操控一切：
我们把Agent（Codex/Claude Code/trae/workbuddy/DeepSeek harness等）视为可插拔的
...
核心原则：**本质协作、不伪造状态、失败即停、先动手后委派、用户保有决策权**。
...
- **管理界面**：经验树、详情、草稿编辑、审计记录、场景与会话入口。
  我认为Experience本质上是一个协作层，因此界面一切从简；
...
experience-main/
  ...
  apps/
    experience-launcher/ # 薄启动器 exe（一次打包；VE5 式启动逻辑）
    experience-server/   # 本地 API 进程（磁盘代码，随改随编）
  ui/www/                # 前端静态页（刷新即生效，无需打包）
```

> 产品定位：Experience 是本体，Agent（含 codex）是“外包 executor”；本仓库
> 为experience_codex仅是因为在开发过程中是以codex Apache2.0作为适配参考，
> 不对codex 做产品级改造/重建；本仓库配套的 Codex fork 只作为可插拔 runtime。
