...
| docs/README.md | **文档地图**：先看决策、契约、产品还是历史实验；避免从讨论稿里拼当前结论 |
| docs/decisions/0001-primary-goals-and-drift-guard.md | **当前主目标与漂移守卫**：区分 stock adapter、router 与 native execution seat |
| docs/decisions/0002-canonical-runtime-two-seams.md | **canonical Runtime**：P1 唯一执行运行时；Codex 只保留 turn 级与 dispatch 级两条缝 |
| docs/decisions/0003-experience-template-binding.md | **三契约与模板绑定**：Exact / Template / Reference；模板绑定后才允许执行 |
| docs/decisions/0004-single-truth-and-verified-learning.md | **单一真相 + 经验来源**：一个解析器/一个写入者；真实轨迹 → 确定性翻译 → 双轨迹归纳 → CANDIDATE → 激活；进程步退出码约束与模板审计 |
| docs/discussions/README.md | **讨论留档**：经验定性、宿主接口、dynamic tool 真机实验与 router 实验结果（25 篇） |
