...
use crate::domain::template::TemplateBindError;
...
pub struct StoreFile {
    ...
    /// Parameterized, non-executable templates. They are bound into concrete
    /// `Experience` values before entering the Gate execution path.
    #[serde(default)]
    pub templates: Vec<ExperienceTemplate>,
...
pub struct ExperienceStore {
    ...
    /// Parameterized templates by insertion order.
    templates: Vec<ExperienceTemplate>,
    ...
    /// template name -> position in `templates`.
    templates_by_name: HashMap<String, usize>,
    /// trigger tool -> active template names.
    active_templates_by_tool: HashMap<String, Vec<String>>,
}
...
impl Default for ExperienceStore {
    fn default() -> Self {
        Self {
            ...
            active_templates_by_tool: HashMap::new(),
        }
    }
}
...
impl ExperienceStore {
    ...
    /// True when `name` addresses any stored artifact (experience *or*
    /// template). Identity-addressed metadata (pin, display name, confidence,
    /// scope) is meaningful for both; executable lookups stay type-specific.
    pub fn contains(&self, name: &str) -> bool {
        self.by_name.contains_key(name) || self.templates_by_name.contains_key(name)
    }

    /// Insert a parameterized template. Templates never enter the exact
    /// experience index; they are bound at decision time.
    pub fn insert_template(&mut self, template: ExperienceTemplate) -> Result<(), StoreError> {
        let issues = template.schema_issues();
        if !issues.is_empty() {
            return Err(StoreError::SchemaInvalid {
        ...
        if self.templates_by_name.contains_key(&template.name) {
            return Err(StoreError::DuplicateName(template.name.clone()));
        }
        self.templates_by_name
            .insert(template.name.clone(), self.templates.len());
        if template.status == ExperienceStatus::Active {
            self.active_templates_by_tool
                .entry(template.trigger.tool.clone())
                .or_default()
    ...
    pub fn template(&self, name: &str) -> Option<&ExperienceTemplate> {
        self.templates_by_name
            .get(name)
            .map(|index| &self.templates[*index])
    }

    pub fn templates(&self) -> &[ExperienceTemplate] {
        &self.templates
    }

    /// ACTIVE templates whose trigger mechanically matches the proposal.
    pub fn templates_for(&self, proposal: &ActionProposal) -> Vec<&ExperienceTemplate> {
        let mut candidates = Vec::new();
        let Some(names) = self.active_templates_by_tool.get(&proposal.tool) else {
            return candidates;
        };
        for name in names {
            if let Some(template) = self.template(name) {
                if template.trigger.matches(proposal) {
    ...
    /// Bind a named template into a concrete Experience. Binding is
    /// deterministic; an unbound required parameter is a hard error, never a
    /// partial execution.
    pub fn bind_template(
        ...
        let template = self
            .template(name)
            .ok_or_else(|| TemplateBindError::MissingParameter(name.to_string()))?;
        template.bind(proposal, cwd)
    }
    ...
    /// Status transition for a *template* (same table as experiences). Kept
    /// separate from `transition_status` so a template can never be mutated
    /// through the exact-experience path and vice versa: the two indexes are
    ...
    pub fn transition_template_status(
        ...
        let index = self
            ...
            .copied()
            .ok_or_else(|| StoreError::NotFound(name.to_string()))?;
        let next = self.templates[index]
            .status
            .transition(action)
            .map_err(StoreError::Transition)?;
        self.templates[index].status = next;
        self.reindex();
        Ok(next)
    }

    /// Remove a template by name and rebuild the indexes.
    pub fn remove_template(&mut self, name: &str) -> Result<(), StoreError> {
        let index = self
            .templates_by_name
            .remove(name)
            .ok_or_else(|| StoreError::NotFound(name.to_string()))?;
    ...
    fn reindex(&mut self) {
        ...
        self.active_templates_by_tool.clear();
        ...
        for (index, template) in self.templates.iter().enumerate() {
            self.templates_by_name.insert(template.name.clone(), index);
            if template.status == ExperienceStatus::Active {
                self.active_templates_by_tool
                    .entry(template.trigger.tool.clone())
                    .or_default()
