...
【Action Gate】← Experience 在这里获得一次同步决策窗口
 ...
 ├─ SATISFIED → 后置条件已经成立：返回成功 no-op，不执行、不放行原 Tool
 └─ MISS      → 正常 Tool 执行
 ↓
...
### 1.1 第三态 `SATISFIED`（2026-09-21 真机补入）

只有 HIT / MISS 时，重入会退化成 MISS：第二次要求“移动已经移动过的文件”，
源不存在，前置不成立，于是任务被错误交回 LLM。M7 真机第一次复现就是
30 次模型请求。
...
前置不成立
  ├─ 后置不成立 → MISS
  └─ 后置已成立 → SATISFIED（成功 no-op）
```

`SATISFIED` 必须满足：

- 所有后置判据都由真实 probe/verification 证明成立；
- 不执行 workflow，不返回 failed；
- 对 turn 级完整任务等价于 `Completed`，对前缀任务等价于“前缀已完成，剩余交 LLM”；
- 仍写 usage/审计，让“重复命中但没有动作”可观测。

### 1.2 模板层：Exact / Template / Reference

Gate 只执行“已经绑定完成的 Experience”。在 Exact 与 Reference 之间新增模板：

```text
ReferenceEntry      非执行，仅供 LLM 借鉴
ExperienceTemplate  参数化动作结构；先确定性绑定
Experience          绑定后的具体实例，才允许进入 Gate
```

模板绑定失败（缺参数、占位符无法解析）= MISS，不允许猜测、不允许部分执行。
只有所有 required 参数绑定成功，模板才编译成一次性 `Experience` 实例并走
同一套 pre/post/verification 流程。
