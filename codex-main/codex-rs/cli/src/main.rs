...
use crate::experience_cmd::ExperienceCli;
...
enum Subcommand {
    ...
    /// Manage the experience store (list / pin / unpin / disable).
    Experience(ExperienceCli),
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Experience(experience_cli)) => {
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
                "experience",
            )?;
            let ExperienceCli { subcommand } = experience_cli;
            experience_cmd::run(subcommand)?;
        }
...
fn unsupported_subcommand_name_for_strict_config(
    ...
    match subcommand {
        ...
        Some(Subcommand::Experience(_)) => Some("experience"),
