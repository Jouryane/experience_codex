    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let services = SessionServices {
                ...
                agent_control,
                experience_runtime: tokio::sync::Mutex::new({
                    // M2-5: persist the experience store under codex_home so
                    // it survives process restarts (management page reads the
                    // same file).
                    let store_dir = config.codex_home.as_path().join("experience");
                    let _ = std::fs::create_dir_all(&store_dir);
                    crate::experience::ExperienceRuntime::load_from_path(
