...
use crate::experience::ProcessActor;
use crate::experience::ProcessStep;
use crate::experience::TraceOutcome;
use crate::experience::TraceRecorder;
...
use crate::session::experience_action_runner::build_reference_prefill;
use crate::session::experience_action_runner::build_text_message_item;
use crate::session::experience_action_runner::execute_experience_if_hit;
use crate::session::experience_distiller::explicit_record_requested;
use crate::session::experience_distiller::run_auto_pipeline;
use crate::session::experience_state_builder::build_experience_state;
use crate::session::experience_state_builder::update_experience_state;
use crate::session::experience_state_builder::user_text;
...
pub(crate) async fn run_turn(
    ...
    // Master switch: EXPERIENCE_ENABLED=0/false disables matching + learning,
    // keeping the original Codex loop behavior unchanged.
    sess.services
        .experience_runtime
        .lock()
        .await
        .set_enabled(crate::experience::enabled());
    ...
    // A1: initial state projection from the turn's first step. Updated inside
    // the loop so task / executed_steps survive across sampling iterations.
    let mut experience_state =
        build_experience_state(turn_context.as_ref(), &first_step_context, &user_input);
    // ③ reference prefill is injected at most once per turn.
    let mut experience_reference_injected = false;
    // M2-1: passive trace recording of LLM tool calls (read-only).
    let mut trace_recorder = TraceRecorder::new();
    let mut llm_sampled = false;
    let mut flushed_tool_actions = 0usize;
    // Canonical M6 turn-level adapter: the same P1 runtime that backs the
    ...
    let canonical_task_text = user_text(&user_input);
    // Learning source: label every recorded observation with the request it
    // served (no-op unless EXPERIENCE_TRACE_OUT is set).
    crate::experience_p1_gate::note_task_text(&canonical_task_text);
    let mut canonical_task_handled = false;
    // M2-2: inherit session-persistent state (registry / deployed / baselines)
    // so experience and LLM share one continuing ledger across turns.
    sess.services
        .experience_runtime
        .lock()
        .await
        .inherit_session(&mut experience_state);
    // M2-3: expire stale state elements and forget unused (non-pinned)
    // experiences at the start of each turn.
    let tick_now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|duration| duration.as_secs())
        .unwrap_or(0);
    sess.services.experience_runtime.lock().await.tick(tick_now);
    ...
    loop {
        ...
        if canonical_task_gate && !canonical_task_handled {
            canonical_task_handled = true;
            match crate::experience_p1_gate::try_task_gate(&canonical_task_text) {
                crate::experience_p1_gate::TaskGateOutcome::Miss => {}
                crate::experience_p1_gate::TaskGateOutcome::Completed {
                    experience,
                    summary,
                } => {
                    let message = format!(
                        "EXPERIENCE TASK GATE: completed by '{experience}' without LLM\n{summary}"
                    );
                    if let Some(item) = build_text_message_item(&message) {
                        sess.record_conversation_items(&turn_context, &[item]).await;
                    }
                    let completed_item = TurnItem::AgentMessage(AgentMessageItem {
                        id: "experience-task-gate".to_string(),
                    ...
                    sess.emit_turn_item_started(&turn_context, &completed_item)
                        .await;
                    sess.emit_turn_item_completed(&turn_context, completed_item)
                        .await;
                    last_agent_message = Some(message);
                    tracing::info!(
                        experience = %experience,
                        "canonical task gate completed the turn; LLM NOT sampled"
                    );
                    break;
                }
                crate::experience_p1_gate::TaskGateOutcome::Handoff {
                    experience,
                    summary,
                } => {
                    let message = format!(
                        "EXPERIENCE TASK PREFIX: '{experience}' executed a verified prefix.\n{summary}\nRemaining task:\n{canonical_task_text}"
                    );
                    if let Some(item) = build_text_message_item(&message) {
                        sess.record_conversation_items(&turn_context, &[item]).await;
                    }
                    tracing::info!(
                        experience = %experience,
                        "canonical task gate executed a prefix; handing remainder to LLM"
                    );
                }
            }
        }

        // V2 world-state seat: after the request-level gate, let experiences
        // whose preconditions already hold run *before* the LLM is consulted.
        // Nothing in the request has to mention them — that is the point.
        if canonical_state_gate && !canonical_state_gate_done {
            canonical_state_gate_done = true;
            for takeover in crate::experience_p1_gate::try_state_gate() {
                let message = format!(
                    "EXPERIENCE STATE TRIGGER: '{}' ran because its preconditions hold in the world{}\n{}",
                    takeover.experience,
                    if takeover.success {
                        ""
                    } else {
                        " (incomplete; control returns to the agent)"
                    },
                    takeover.summary
                );
                if let Some(item) = build_text_message_item(&message) {
                ...
                tracing::info!(
                    experience = %takeover.experience,
                    success = takeover.success,
                    "state-triggered experience ran before sampling"
                );
            }
        }

        // LEGACY EXPERIENCE DECISION POINT (explicit compatibility only).
        // The canonical P1 runtime owns the decision whenever a Gate store is
        // configured; this branch requires EXPERIENCE_LEGACY_OUTER=1.
        if legacy_outer {
            // EXPERIENCE DECISION POINT — pseudo-parallel fast path.
            // A1 refresh (tools / detections / continuity) + decide; ①/② execute
            // the experience workflow; `Completed` means the LLM is skipped.
            update_experience_state(
                &mut experience_state,
                turn_context.as_ref(),
                step_context.as_ref(),
            );
            let experience_decision = sess
                .services
                .experience_runtime
                .lock()
                .await
                .decide_for_state(&experience_state);
            // M2-2: write the decision's behavior mode back into the state
            // (①/② → Reflex, ③/④ → Deliberation, AbortOrAsk → Conflict).
            experience_state.execution_mode = experience_decision.execution_mode();
            // M2-4: record who acts at this decision point (task process log).
            let (process_actor, process_action) = match &experience_decision {
                ControlDecision::ExperienceOnly(id) => {
                    (ProcessActor::Experience, format!("经验接管 {id}"))
                }
                ControlDecision::ExperienceFirst(id) => {
                    (ProcessActor::Experience, format!("经验执行·LLM 检查 {id}"))
                }
                ControlDecision::Reference(id) => {
                    (ProcessActor::Experience, format!("经验参考 {id}"))
                }
                ControlDecision::Delegate => (ProcessActor::Llm, "LLM 主导".to_string()),
                ControlDecision::AbortOrAsk => {
                    (ProcessActor::Experience, "冲突：停止自动执行".to_string())
                }
            };
            experience_state.process_log.push(ProcessStep {
                actor: process_actor,
                action: process_action,
                detail: String::new(),
            });
            // Tool infra for ①/② execution (sanctioned path: approvals, sandbox,
            // tool recording) and the ③ reference injection.
            let tool_runtime = ToolCallRuntime::new(
                Arc::clone(&sess),
                Arc::clone(&step_context),
                Arc::clone(&turn_diff_tracker),
            );
            let experience_cancellation = cancellation_token.child_token();
            match &experience_decision {
                ControlDecision::Delegate => {}
                ControlDecision::Reference(experience_id) => {
                    if !experience_reference_injected {
                        if let Some(experience) = sess
                            .services
                            .experience_runtime
                            .lock()
                            .await
                            ...
                            .cloned()
                        {
                            let prefill = build_reference_prefill(&experience_state, &experience);
                            if let Some(item) = build_text_message_item(&prefill) {
                                sess.record_conversation_items(&turn_context, &[item]).await;
                                experience_reference_injected = true;
                                tracing::info!(
                                    ?experience_id,
                                    "injected experience reference into prompt"
                                );
                                let task = experience_state
                                    .task
                                    .as_ref()
                                    .map(|task| task.description.clone());
                                sess.services.experience_runtime.lock().await.record_usage(
                                    experience_id,
                                    "reference",
                                    None,
                ...
                ControlDecision::AbortOrAsk => {
                    tracing::warn!(
                        "experience conflict: auto-execution stop is not wired; falling back to LLM"
                    );
                }
                ControlDecision::ExperienceOnly(_) | ControlDecision::ExperienceFirst(_) => {
                    match execute_experience_if_hit(
                        &sess,
                        &mut experience_state,
                        &experience_decision,
                        Some(tool_runtime),
                    ...
                    .await
                    {
                        Some(ExecutionOutcome::Completed) => {
                            // M2-4: echo the result (with the detailed process)
                            // so the user can see what experience did.
                            let message = experience_state.render_result_message();
                            if let Some(item) = build_text_message_item(&message) {
                                sess.record_conversation_items(&turn_context, &[item]).await;
                            }
                            tracing::info!(
                                ?experience_decision,
                                "experience completed the task; skipping LLM"
                            );
                            break;
                        }
                        Some(outcome) => {
                            tracing::warn!(
                                ?experience_decision,
                                ?outcome,
                                "experience did not complete; handing to LLM with execution state"
                            );
                        }
                        None => {
                            tracing::warn!(
                                ?experience_decision,
                                "experience missing from store; falling back to LLM"
                            );
                        }
                    }
                }
        ...
        llm_sampled = true;
        // M2-4: record the LLM sampling step in the task process log.
        experience_state
            .process_log
            .push(ProcessStep::llm("采样", "LLM 推理"));
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                // M2-4: flush this sampling request's tool calls into the task
                // process log (chronological per loop iteration).
                let actions = trace_recorder.actions();
                for action in &actions[flushed_tool_actions..] {
                    experience_state
                        .process_log
                        .push(ProcessStep::tool(&action.name, action.args.to_string()));
                }
                flushed_tool_actions = actions.len();
    ...
    // M2-1: after a successful LLM-driven turn, compile the trace and let the
    // learner decide whether it is repeatable. Experience-only turns (zero
    // LLM) produce no trace. Drafts then enter the automatic pipeline:
    // distill (messy traces via one small LLM compile call) → validate
    // (dry-run at the tool boundary) → activate. Zero user interaction.
    if legacy_outer && llm_sampled && !trace_recorder.is_empty() {
        let task_text = user_text(&user_input);
        // Silence-audit fix: attach the state snapshot and the agent's final
        // observed evidence so the learner/distiller sees *how the task was
        // completed*, not just the flat action list.
        let environment_snapshot = Some(experience_state.snapshot_for_learning());
        let trace = trace_recorder.into_trace_with_context(
            task_text.clone(),
        ...
        // User release channel: when the user explicitly asks to remember
        // this procedure, force-record the trace (bypassing the
        // no-improvement gate; quality gates still apply).
        let explicit = explicit_record_requested(&task_text);
        let draft = if explicit {
            sess.services
                .experience_runtime
                .lock()
                .await
            ...
            sess.services
                ...
                .lock()
                .await
                .observe_trace(&trace, std::env::var("EXPERIENCE_AUTO_CONFIRM").is_ok())
        };
        if let Some(draft) = draft {
            tracing::info!(
                experience_id = %draft.experience.id,
                "learner compiled an experience draft from an LLM trace"
            );
            run_auto_pipeline(
                &sess,
                &turn_context,
    ...
    if legacy_outer {
        // M2-2: persist the turn's final state as the next turn's baseline.
        sess.services
            .experience_runtime
            .lock()
            .await
            .commit_session(&experience_state);
    }
...
async fn drain_in_flight(
    ...
    while let Some(res) = in_flight.next().await {
        match res {
            Ok(envelope) => {
                record_trace_outcome(trace_recorder, &envelope.item);
...
/// Correlate a completed tool result (by `call_id`) back to the recorded
/// trace step and store its outcome + output summary. This is the missing
/// per-step success evidence identified by the silence audit.
fn record_trace_outcome(trace_recorder: &mut TraceRecorder, item: &ResponseItem) {
    match item {
        ResponseItem::FunctionCallOutput {
            call_id, output, ..
        } => {
            if let Some(call_id) = call_id {
                trace_recorder.record_outcome(
                    call_id,
                    output.success != Some(false),
                    output
                        .body
                        .to_text()
                        .map(|text| truncate_trace_summary(&text)),
                );
            }
        }
...
fn truncate_trace_summary(text: &str) -> String {
    const MAX_TRACE_SUMMARY_CHARS: usize = 800;
    if text.chars().count() <= MAX_TRACE_SUMMARY_CHARS {
        text.to_string()
    } else {
        text.chars()
            .take(MAX_TRACE_SUMMARY_CHARS)
            .collect::<String>()
    }
}
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone(mut item) => {
                ...
                // M2-1 fix: record the function call only when the item is
                // complete (OutputItemDone) so `arguments` is fully populated.
                // Read-only; never changes the execution path.
