...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Ok(Some(call)) => {
            ...
            // M4: real Experience Gate (embedded Runtime, Model A). Position:
            // after build_tool_call parsed the call successfully, before the
            // tool is dispatched (副作用前). On HIT the original tool is NOT
            // dispatched; a legal FunctionCallOutput is injected and the loop
            // continues. MISS / unavailable runtime falls through.
            if let Some((gate_call_id, gate_payload)) =
                crate::experience_p1_gate::try_experience_gate(&item)
            {
            ...
            // V2 learning source: the model chose this call and the Gate did
            // not replace it, so it is a real observation of how the task was
            // done. Recorded when EXPERIENCE_TRACE_OUT is set; otherwise a
            // single env lookup and nothing else.
            crate::experience_p1_gate::record_observed_call(&item);
