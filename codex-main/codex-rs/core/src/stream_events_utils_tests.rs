...
#[tokio::test]
async fn gate_hit_runs_embedded_experience_and_skips_tool_dispatch() {
    // M4 (Model A): deterministic FunctionCall -> real Experience Gate HIT ->
    // embedded P1 runtime executes the create_probe_file experience (writes
    // probe.txt) -> original tool executor NOT dispatched -> legal
    // FunctionCallOutput injected -> Codex loop continues. No LLM / API /
    // login involved.
    let nanos = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
        .as_nanos();
    let work_dir = std::env::temp_dir().join(format!("exp-m4-gate-{nanos}"));
    std::fs::create_dir_all(&work_dir).unwrap();
    let fixture = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../../experience-main/experiences/create_probe_file.json"
    );
    let experience = std::fs::read_to_string(fixture).expect("P1 fixture must exist");
    let envelope = format!(r#"{{"schema_version":1,"experiences":[{experience}]}}"#);
    let store_path = work_dir.join("store.json");
    std::fs::write(&store_path, envelope).unwrap();
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            unsafe {
                match &self.store {
                    Some(value) => std::env::set_var("EXPERIENCE_GATE_STORE", value),
                    None => std::env::remove_var("EXPERIENCE_GATE_STORE"),
                }
                match &self.cwd {
                    Some(value) => std::env::set_var("EXPERIENCE_GATE_CWD", value),
                    None => std::env::remove_var("EXPERIENCE_GATE_CWD"),
                }
            }
        }
    }
    let guard = EnvGuard {
        store: std::env::var_os("EXPERIENCE_GATE_STORE"),
        cwd: std::env::var_os("EXPERIENCE_GATE_CWD"),
    };
    unsafe {
        std::env::set_var("EXPERIENCE_GATE_STORE", &store_path);
        std::env::set_var("EXPERIENCE_GATE_CWD", &work_dir);
    }
    crate::experience_p1_gate::reset_gate_runtime_for_test();
    ...
    let call = ResponseItem::FunctionCall {
        ...
        namespace: None,
        arguments: serde_json::json!({ "cmd": "please create probe file now" }).to_string(),
        call_id: "call-gate-1".to_string(),
        encrypted_function_args: None,
        internal_chat_message_metadata_passthrough: None,
    };

    let output = handle_output_item_done(&mut ctx, call, /*previously_active_item*/ None)
        .await
        .expect("gate hit should complete without error");

    // 1) original tool executor was NOT dispatched/queued
    assert!(
        output.tool_future.is_none(),
        "gate HIT must not dispatch the tool"
    );
    // 2) the loop continues (model sees a result and keeps going)
    assert!(
        output.needs_follow_up,
        "gate HIT must keep the Codex loop alive"
    );
    // 3) the embedded experience actually ran and verified its postcondition
    let probe_path = work_dir.join("probe.txt");
    assert_eq!(
        std::fs::read_to_string(&probe_path).expect("experience must write probe.txt"),
        "EXPERIENCE_GATE_SUCCESS"
    );
    // 4) a legal FunctionCallOutput was injected into the conversation
    let history = ctx
        .sess
        .clone_history()
        .await
        .for_prompt(&ctx.turn_context.model_info().input_modalities);
    let injected = history.iter().any(|item| {
        serde_json::to_string(item)
            .map(|text| text.contains("takeover completed"))
            .unwrap_or(false)
    });
    assert!(injected, "expected injected FunctionCallOutput in history");
    crate::experience_p1_gate::reset_gate_runtime_for_test();
    drop(guard);
