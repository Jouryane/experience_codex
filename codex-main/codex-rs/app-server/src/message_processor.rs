...
use crate::request_processors::ExperienceRequestProcessor;
...
pub(crate) struct MessageProcessor {
    ...
    experience_processor: ExperienceRequestProcessor,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let experience_processor = ExperienceRequestProcessor::new();
    ...
    async fn handle_initialized_client_request(
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::AppsInstalled { params, .. } => self
                ...
                .await
                .map(|response| Some(response.into())),
            ClientRequest::ExperienceList { params, .. } => {
                self.experience_processor.experience_list(params).await
            }
            ClientRequest::ExperienceDetail { params, .. } => {
                self.experience_processor.experience_detail(params).await
            }
            ClientRequest::ExperiencePin { params, .. } => {
                self.experience_processor.experience_pin(params).await
            }
            ClientRequest::ExperienceUnpin { params, .. } => {
                self.experience_processor.experience_unpin(params).await
            }
            ClientRequest::ExperienceActivate { params, .. } => {
                self.experience_processor.experience_activate(params).await
            }
            ClientRequest::ExperienceRevalidate { params, .. } => {
                self.experience_processor.experience_revalidate(params).await
            }
            ClientRequest::ExperienceDisable { params, .. } => {
                self.experience_processor.experience_disable(params).await
            }
            ClientRequest::ExperienceDelete { params, .. } => {
                self.experience_processor.experience_delete(params).await
            }
            ClientRequest::ExperienceExport { params, .. } => {
                self.experience_processor.experience_export(params).await
            }
            ClientRequest::ExperienceImport { params, .. } => {
                self.experience_processor.experience_import(params).await
            }
            ClientRequest::ExperienceEditAsDraft { params, .. } => {
                self.experience_processor.experience_edit_as_draft(params).await
            }
            ClientRequest::ExperienceAdoptDraft { params, .. } => {
                self.experience_processor.experience_adopt_draft(params).await
            }
            ClientRequest::ExperienceUpdateMeta { params, .. } => {
                self.experience_processor.experience_update_meta(params).await
            }
            ClientRequest::ExperienceUpdateControl { params, .. } => {
                self.experience_processor.experience_update_control(params).await
            }
